...
매니페스트에는 아직 `dependencies: { "soksak-plugin-git-core": "^0.1.0" }` 가 남아 있다. 이건 이
플러그인의 선택이 **아니다**: 코어의 cross-plugin 게이트가 대상 **플러그인 id** 가 `dependencies`
에 있을 때만 호출을 허용한다. **코어가 소비자 축 계약-핀 선언을 받아들이면 이 줄을 지운다** —
코드는 이미 구현체 이름을 부르지 않으므로 코드 변경은 없다.
